...
function hasReservationId(req, res, next) {
  ...
  if (!table) return next({ status: 400, message: "Must have data property" });
  if (!table.reservation_id) return next({ status: 400, message: "Must have reservation_id" });
...
function isAlreadySeated(req, res, next) {
  ...
  if (status === "seated") return next({ status: 400, message: "Reservation is already seated" });
...
async function tableIsValid(req, res, next) {
  ...
  if (currentTable.reservation_id) return next({ status: 400, message: "Table occupied." });
...
async function tableIsOccupied(req, res, next) {
  ...
  if (!table) return next({ status: 404, message: `Table ${table_id} not found` });
  if (!table.reservation_id) return next({ status: 400, message: "Table is not occupied" });
