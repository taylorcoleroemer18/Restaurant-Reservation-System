...
function isValidReservation(req, res, next) {
  ...
  if (!reservation) return next({ status: 400, message: `Must have data property.` });

  VALID_RESERVATION_FIELDS.forEach((field) => {
    if (!reservation[field]) return next({ status: 400, message: `${field} field required` });
...
function isNotOnTuesday(req, res, next) {
  ...
  if (date.getDay() === 2) return next({ status: 400, message: "Location is closed on Tuesdays" });
...
function isInTheFuture(req, res, next) {
  ...
  if (date < today) return next({ status: 400, message: "Must be a future date" });
...
function isValidStatus(req, res, next) {
  ...
  if (!VALID_STATUSES.includes(status)) return next({ status: 400, message: "Status unknown." });
...
async function list(req, res) {
  ...
  if (mobile_number) reservations = await service.search(mobile_number);
  else reservations = date ? await service.listByDate(date) : await service.list();
